...
function renderPhaseNav() {
  ...
  scenario.phases.forEach((p, i) => {
    ...
    if (editMode) {
      // 編集モード: ボタンでタブ切替 ＋ 下にラベル編集inputを表示
      ...
      html += `<input style="width:80px;font-size:10px;padding:3px 6px;border:1px solid var(--border-2);border-radius:6px;background:var(--surface);color:var(--text);text-align:center;outline:none" value="${esc(p.label)}" oninput="scenario.phases[${i}].label=this.value;scheduleSave();renderPhaseNav()">`;
      html += `</div>`;
...
function renderEval() {
  ...
  sorted.forEach((item) => {
    ...
    if (e) {
      html += `<div class="eval-item eval-edit-box" data-eid="${esc(item.id)}">
        <button class="eval-edit-del" data-del-eval="${esc(item.id)}" title="削除">
        ...
        <div class="card-label" style="margin-bottom:8px">項目 ${origIdx+1}</div>
        <input class="edit-input" style="margin-bottom:8px;font-weight:700" value="${esc(item.task)}"
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Supabase 初期化（config.js が設定済みなら）
  if (window.supabase) {
    initSupabase();
  } else {
    setSyncStatus('', 'ローカル保存のみ');
  }
});
